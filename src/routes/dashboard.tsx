...
function DashboardPage() {
  ...
  const stats = [
    ...
    { icon: Clock, label: "Last activity", value: activities.length ? timeAgo(activities[0].timestamp) : "N/A" },
      ...
      <main className="flex-1 py-8 sm:py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-10">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="mt-1 font-display text-2xl sm:text-3xl md:text-4xl font-bold">Welcome back 👋</h1>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-0.5">Summarize, translate, and chat with any PDF using AI</p>
              </div>
              <Link to="/tools">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-primary-foreground/80">Just ₹50/month — summarize, translate, batch process</p>
              </div>
              <Link to="/pricing">
          ...
          <div>
            <div className="flex items-end justify-between mb-5">
              ...
              <Link to="/tools" className="text-sm font-semibold text-primary hover:underline">All tools →</Link>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              {activities.length > 0 && (
                <button onClick={() => { clearActivities(); setActivities([]); }} className="text-xs text-muted-foreground hover:text-foreground transition flex items-center gap-1">
