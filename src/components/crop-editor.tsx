...
export function CropEditor({ file }: { file: File }) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      setSel({
        x1: Math.min(s.mx, mx), y1: Math.min(s.my, my),
        x2: Math.max(s.mx, mx), y2: Math.max(s.my, my),
      });
    };
  ...
  return (
    <div className={fullscreen ? "fixed inset-0 z-50 bg-background flex flex-col" : "space-y-3"}>
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-card px-3 sm:px-4 py-2 shadow-soft shrink-0">
        <div className="flex items-center gap-1.5 text-sm"><Crop className="h-4 w-4 text-primary" /> <span className="font-medium">Drag to select crop area</span></div>
      ...
      <div className="flex-1 overflow-auto bg-neutral-800/50 flex items-center justify-center p-4">
        <div className="relative inline-block shadow-2xl" style={{ maxHeight: "65vh" }}>
          <img ref={imgRef} src={pg.dataUrl} alt="" className="block max-h-[65vh] object-contain" draggable={false} />
          {/* Selection overlay */}
          <div ref={overlayRef} className="absolute inset-0 cursor-crosshair" onMouseDown={handleMouseDown}>
            {/* Dark overlay areas */}
            <div className="absolute bg-black/50" style={{ top: 0, left: 0, right: 0, height: `${sel.y1 * 100}%` }} />
            <div className="absolute bg-black/50" style={{ bottom: 0, left: 0, right: 0, height: `${(1 - sel.y2) * 100}%` }} />
            <div className="absolute bg-black/50" style={{ top: `${sel.y1 * 100}%`, left: 0, width: `${sel.x1 * 100}%`, height: selH }} />
            <div className="absolute bg-black/50" style={{ top: `${sel.y1 * 100}%`, right: 0, width: `${(1 - sel.x2) * 100}%`, height: selH }} />
            {/* Selection border */}
            <div className="absolute border-2 border-primary border-dashed"
              style={{ left: `${sel.x1 * 100}%`, top: `${sel.y1 * 100}%`, width: selW, height: selH }} />
            {/* Corner handles */}
            {[
              ...
              <div key={i} className="absolute w-3 h-3 bg-primary rounded-full border-2 border-white -translate-x-1/2 -translate-y-1/2"
