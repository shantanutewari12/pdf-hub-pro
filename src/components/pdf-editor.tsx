...
export function PdfEditor({ file }: { file: File }) {
  ...
  useEffect(() => {
    ...
    c.width = pg.width * zoom;
    c.height = pg.height * zoom;
  ...
  const addAnnotation = (a: Annotation) =>
    updatePage((p) => ({ ...p, annotations: [...p.annotations, a] }));
  const updateAnnotation = (id: string, u: Partial<Annotation>) =>
    updatePage((p) => ({
      ...p,
      annotations: p.annotations.map((a) => (a.id === id ? ({ ...a, ...u } as Annotation) : a)),
    }));
        ...
        for (const ann of pages[i].annotations) {
          if (ann.type === "text") {
            ...
            page.drawText(ann.text, {
              x: ann.x * sx,
              y: height - ann.y * sy - ann.fontSize * sy,
            ...
            page.drawImage(emb, {
              x: ann.x * sx,
              y: height - ann.y * sy - ann.height * sy,
              width: ann.width * sx,
  ...
  if (!pages.length)
    return <p className="text-center text-muted-foreground p-8">No pages found.</p>;

  const pg = pages[currentPage];
  const wrapperClass = fullscreen ? "fixed inset-0 z-50 bg-background flex flex-col" : "space-y-3";
  ...
  return (
    <div className={wrapperClass}>
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 border-b border-border bg-card px-2 sm:px-4 py-2 shadow-soft shrink-0">
        ...
        <Button size="sm" variant="ghost" onClick={() => setZoom((z) => Math.min(3, z + 0.15))}>
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-auto bg-neutral-800/50 flex items-start justify-center p-4"
        style={fullscreen ? {} : { maxHeight: "70vh" }}
      >
        <div
          className="relative inline-block shadow-2xl"
          style={{ width: pg.width * zoom, height: pg.height * zoom }}
        >
          <canvas
            ref={canvasRef}
            onClick={handleCanvasClick}
            className={`block ${tool === "text" ? "cursor-crosshair" : "cursor-default"}`}
          />
          ...
          {pg.annotations.map((ann) => {
            if (ann.type === "text") {
              return (
                ...
                >
                  {ann.editing ? (
                    <div
                      className="flex flex-col gap-1 bg-card border border-primary rounded-lg p-2 shadow-xl"
                      style={{ minWidth: 180 }}
                    >
                      <textarea
                        autoFocus
                        ...
                        onBlur={(e) => {
                          ...
                          updateAnnotation(ann.id, { text: v, editing: false });
                        }}
                        onKeyDown={(e) => {
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          value={ann.fontSize}
                          onChange={(e) =>
                            updateAnnotation(ann.id, { fontSize: Number(e.target.value) })
                          }
                          className="text-xs bg-muted rounded px-1"
                        >
                          {[10, 12, 14, 16, 20, 24, 28, 36, 48].map((s) => (
                    ...
                    <div
                      className={`px-1 rounded cursor-move select-none ${draggingId === ann.id ? "ring-2 ring-primary" : "hover:ring-1 hover:ring-primary/50"}`}
            ...
            if (ann.type === "image") {
              return (
                <div
                  key={ann.id}
                  className={`absolute group cursor-move ${draggingId === ann.id ? "ring-2 ring-primary" : "hover:ring-1 hover:ring-primary/40"}`}
                  style={{
                    left: ann.x * zoom,
                    top: ann.y * zoom,
                ...
                >
                  ...
                  {/* Resize handle */}
                  <div
                    className="absolute bottom-0 right-0 w-5 h-5 cursor-se-resize bg-primary/80 rounded-tl-md"
                    onMouseDown={(e) => startResize(e, ann as ImageAnnotation)}
                    onTouchStart={(e) => startResize(e, ann as ImageAnnotation)}
                  >
                    <svg viewBox="0 0 10 10" className="w-3 h-3 m-1 text-white">
                      <path
                        d="M0 10L10 0M4 10L10 4M8 10L10 8"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        fill="none"
                      />
