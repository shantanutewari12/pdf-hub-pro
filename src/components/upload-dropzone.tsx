...
import { UploadCloud, FileText, X, Lock, Download, CheckCircle2, Eye, FileSearch, RefreshCw, Brain, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { toolHandlers, downloadResult, generatePdfPreview, type ProcessResult } from "@/lib/pdf-processors";
...
export function UploadDropzone({ toolName = "files", toolSlug, accept, multiple = true }: Props) {
  ...
  const addFiles = useCallback(
    (list: FileList | null) => {
      ...
      setFiles((prev) => (effectiveMultiple ? [...prev, ...Array.from(list)] : Array.from(list).slice(0, 1)));
    },
    [effectiveMultiple]
  );
  ...
  const start = async () => {
    ...
    if (handler?.minFiles && files.length < handler.minFiles) {
      toast.error(`Please select at least ${handler.minFiles} files`); return;
    }
    setProcessing(true);
    setProgress(10);
    setShowPreview(false);
    ...
    try {
      if (handler) {
        ...
        for (let i = 10; i <= 100; i += 10) { await new Promise((r) => setTimeout(r, 80)); setProgress(i); }
        clearInterval(tick);
  ...
  const handleRetranslate = async (newLang: string) => {
    ...
    try {
      ...
      setResult(res);
      if (res.text) { setPreviewUrl(null); setShowPreview(true); }
      playSuccessSound();
  ...
  const renderPrompt = () => {
    ...
    if (handler.promptType === "select" && handler.promptOptions) {
      return (
        <div className="mt-4">
          ...
          <select value={promptValue} onChange={(e) => setPromptValue(e.target.value)} disabled={processing}
            ...
            {handler.promptOptions.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
          </select>
        </div>
      );
    }
    return (
      <div className="mt-4">
        <label className="block text-sm font-medium mb-1.5">{handler.promptLabel}</label>
        <input type="text" value={promptValue} onChange={(e) => setPromptValue(e.target.value)} placeholder={handler.promptPlaceholder ?? ""}
          disabled={processing} className="w-full h-10 rounded-xl border border-border bg-background px-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
      </div>
    );
  };
    ...
    <div className="w-full space-y-4">
      ...
      >
        <div className="flex flex-col items-center gap-3 sm:gap-4">
          <motion.div animate={{ y: drag ? -6 : 0 }} className={`flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-2xl shadow-elevated ${isAI ? "bg-gradient-to-br from-primary via-emerald-500 to-teal-500" : "bg-gradient-emerald"}`}>
            {isAI ? <Brain className="h-7 w-7 sm:h-8 sm:w-8 text-primary-foreground" strokeWidth={2} /> : <UploadCloud className="h-7 w-7 sm:h-8 sm:w-8 text-primary-foreground" strokeWidth={2} />}
          </motion.div>
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-muted-foreground">or click to browse — up to 100MB</p>
          </div>
          <label>
            <input type="file" className="hidden" accept={effectiveAccept} multiple={effectiveMultiple} onChange={(e) => addFiles(e.target.files)} />
      ...
      <AnimatePresence>
        {files.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="rounded-2xl border border-border bg-card p-3 sm:p-4 shadow-soft">
            {/* File list */}
            <div className="space-y-2">
              {files.map((f, i) => (
                <div key={i} className="flex items-center gap-2 sm:gap-3 rounded-lg bg-muted/40 px-2 sm:px-3 py-2">
                  ...
                  <button onClick={() => setFiles((p) => p.filter((_, idx) => idx !== i))} className="p-1 rounded hover:bg-background" disabled={processing}>
            ...
            {processing && isAI && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 rounded-2xl bg-gradient-to-r from-primary/5 via-accent/5 to-primary/5 border border-primary/20 p-5 text-center">
                <div className="relative mx-auto w-16 h-16 mb-4">
                  ...
                  {/* Inner pulse */}
                  <motion.div className="absolute inset-2 rounded-full bg-primary/10"
                ...
                <motion.p key={aiPhrase} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-sm font-medium text-primary">
                  {AI_PHRASES[aiPhrase]}
                </motion.p>
            ...
            {processing && !isAI && (
              <div className="mt-4">
                <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
                  <span>Processing securely…</span><span>{progress}%</span>
                </div>
                <div className="h-2 rounded-full bg-muted overflow-hidden">
                  <motion.div className="h-full bg-gradient-emerald" animate={{ width: `${progress}%` }} transition={{ ease: "linear" }} />
                </div>
              </div>
            )}
            ...
            {/* Result card */}
            {result && !processing && (
              <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Button size="sm" variant="outline" onClick={() => handlePreview(result)} disabled={generatingPreview} className="gap-1.5 flex-1 sm:flex-none">
                      {generatingPreview ? <span className="animate-spin h-3 w-3 border-2 border-current border-t-transparent rounded-full" /> : <Eye className="h-4 w-4" />}
                      Preview
                    </Button>
                    <Button size="sm" onClick={() => handleDownload(result)} className="bg-gradient-emerald text-primary-foreground hover:opacity-90 gap-1.5 flex-1 sm:flex-none">
                ...
                {handler?.supportRetranslate && handler.promptOptions && (
                  <div className="mt-3 flex flex-col sm:flex-row items-start sm:items-center gap-2 pt-3 border-t border-primary/10">
                    ...
                    <select value={retranslateLang} onChange={(e) => handleRetranslate(e.target.value)} disabled={retranslating}
                    ...
                    {retranslating && <span className="text-xs text-muted-foreground">Translating…</span>}
                  </div>
                )}
                  ...
                  {showPreview && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-3 overflow-hidden">
                      <div className="rounded-xl border border-border overflow-hidden bg-background">
                        {previewUrl && <img src={previewUrl} alt="Preview" className="w-full object-contain max-h-[400px] sm:max-h-[500px]" />}
                        {!previewUrl && result.text && (
                          <div className="p-3 sm:p-4">
                            ...
                            <pre className="whitespace-pre-wrap text-xs sm:text-sm leading-relaxed font-sans text-foreground max-h-60 sm:max-h-80 overflow-y-auto">{result.text}</pre>
                          </div>
                        )}
                        {!previewUrl && !result.text && (
                      ...
                      <button onClick={() => setShowPreview(false)} className="mt-2 text-xs text-muted-foreground hover:text-foreground transition">Hide preview</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
            ...
            <div className="mt-4 flex gap-2 justify-end">
              <Button variant="ghost" onClick={() => { setFiles([]); setResult(null); setPreviewUrl(null); setShowPreview(false); }} disabled={processing}>Clear</Button>
              <Button onClick={start} disabled={processing} className="bg-gradient-emerald text-primary-foreground hover:opacity-90">
