...
export function RotateEditor({ file }: { file: File }) {
  ...
  const rotateCurrentPage = (deg: number) => {
    setPages((prev) =>
      prev.map((p, i) =>
        i === currentPage ? { ...p, rotation: (p.rotation + deg + 360) % 360 } : p,
      ),
    );
  };
  ...
  return (
    <div className={fullscreen ? "fixed inset-0 z-50 bg-background flex flex-col" : "space-y-3"}>
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-card px-3 sm:px-4 py-2 shadow-soft shrink-0">
        <Button size="sm" variant="outline" onClick={() => rotateCurrentPage(-90)}>
          <RotateCcw className="h-4 w-4 mr-1" /> -90°
        </Button>
        <Button size="sm" variant="outline" onClick={() => rotateCurrentPage(90)}>
          <RotateCw className="h-4 w-4 mr-1" /> +90°
        </Button>
        <Button size="sm" variant="outline" onClick={() => rotateCurrentPage(180)}>
          <FlipHorizontal2 className="h-4 w-4 mr-1" /> 180°
        </Button>
        <div className="h-5 w-px bg-border mx-0.5 hidden sm:block" />
        <Button size="sm" variant="ghost" onClick={() => rotateAllPages(90)} className="text-xs">
          All +90°
        </Button>
      ...
      <div className="flex-1 overflow-auto bg-neutral-800/50 flex items-center justify-center p-4">
        <div
          ...
          style={{ transition: "transform 0.3s", transform: `rotate(${pg.rotation}deg)` }}
        >
          <img
            src={pg.canvasDataUrl}
